(function(){
  ...
  function extractVar(source,name){var range=findVarRange(source,name);return JSON.parse(source.slice(range.valueStart,range.end));}
  function replaceVar(source,name,value){var range=findVarRange(source,name);return source.slice(0,range.start)+'var '+name+'='+JSON.stringify(value)+source.slice(range.end);}
  function validateArray(row,field){if(!Array.isArray(row[field])||row[field].length!==12)throw new Error('Array '+field+' invalido em '+row.contract+' | '+row.name);}
  ...
  function loadOverrides(){
    ...
    dataPromise=Promise.all([0,1,2,3,4,5,6,7].map(function(i){
      return nativeFetch('data/direct-overrides.part'+i+'?v=financefix20260731',{cache:'no-store'}).then(function(response){
    ...
    }).then(validateRows).then(function(rows){window.__SINAPE_DATA_ROWS__=rows.length;return rows;});
    return dataPromise;
  }

  function twelve(values,fill){
    var result=Array.isArray(values)?values.slice(0,12):[];
  ...
  function mergeSource(source,rows){
    ...
    var contracts=extractVar(source,'contractData'),bySource={},byKey={},occurrence={},used=[],matched=0;
    ...
    rows.forEach(function(row){
      var targetEntry=row.sourceId?bySource[String(row.sourceId)]||null:null;
      if(!targetEntry){
        var key=row.contract+'|'+normalize(row.name),index=occurrence[key]||0;
        targetEntry=(byKey[key]||[])[index]||null;occurrence[key]=index+1;
      }
      ...
      var target=targetEntry.item,price=Number(row.price||0);
      target.sourceId=row.sourceId;target.unit=row.unit;target.price=price;
      target.contractual=Number(row.contractual||0);target.base=Number(row.base||0);target.balance=Number(row.balance||0);
      target.plan=twelve(row.plan,0);target.exec=twelve(row.exec,0);target.orders=twelve(row.orders,'Sem serviços');
      target.planValue=Array(12).fill(0);target.execValue=Array(12).fill(0);
      for(var month=0;month<6;month++){
        target.planValue[month]=Number(target.plan[month]||0)*price;
        target.execValue[month]=Number(target.exec[month]||0)*price;
      }
      for(var future=6;future<12;future++){
        target.plan[future]=0;target.exec[future]=0;target.planValue[future]=0;target.execValue[future]=0;target.orders[future]='Sem serviços';
      }
      matched++;
    });

    if(matched!==650)throw new Error('Itens vinculados divergentes: '+matched);
    window.__SINAPE_PLANEJADO_EXECUTADO_SNAPSHOT__={items:matched,monthsWithData:6,futureMonthsZeroed:6,financeRule:'quantidade x preco unitario'};
    ...
    if(url.indexOf('Cronogramas%20Planejamento/index.html')>=0||url.indexOf('Cronogramas Planejamento/index.html')>=0){
      return nativeFetch(input,Object.assign({},options||{},{cache:'no-store'})).then(function(response){
        ...
        return loadOverrides().then(function(rows){
          ...
          window.__SINAPE_DATA_READY__=true;window.__SINAPE_DATA_ERROR__='';
          document.documentElement.setAttribute('data-sinape-data','ready');
          return new Response(merged,{status:200,headers:{'Content-Type':'text/html; charset=utf-8','X-SINAPE-DATA':'ready','X-SINAPE-FINANCE':'quantity-times-price'}});
        }).catch(function(error){
          console.error('[SINAPE] Falha ao aplicar base atualizada:',error);
          window.__SINAPE_DATA_READY__=false;window.__SINAPE_DATA_ERROR__=String(error&&error.message||error);
  ...
  loadOverrides().catch(function(error){console.error('[SINAPE] Falha no pre-carregamento da base:',error);});
})();
